...
def get_private_messages(request):
    ...
    try:
        ...
        messages.error(request, 'Please buy a subscription to access private messaging.')
        return redirect(reverse('checkout'))
    
    try: 
        if request.user.premiumuser:
            private_messages = PrivateMessages.objects.filter(to_user=request.user)
            ...
            return render(request, 'private_messages/private_messages.html', context)
        ...
        if request.user.premiumuser:
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    form.from_user = user   
                    form.save()
                    messages.success(request, 'Message sent! Please note, if user does not'
                                     'have a premium subscription, they will not see messages.')
            ...
            return render(request, 'private_messages/send_private_message.html', context)
